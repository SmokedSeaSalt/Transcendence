...
test('successful registration calls fetch with credentials', async () => {
	(globalThis.fetch as any) = vi.fn().mockResolvedValue({ ok: true, json: async () => ({ id: 1 }) });
...
test('valid email + invalid password shows password validation and does not call fetch', async () => {
	...
	expect(await screen.findByText(/Password must be at least 8 characters|Password must contain/i)).toBeInTheDocument();
