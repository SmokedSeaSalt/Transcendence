...
export const roomStore = {
  ...
  addUser: (roomId: string, userId: string, name: string, databaseUserId: number | null): void => {
    const room = rooms.get(roomId);
    if (!room) return;
	// add user to room
	room.users[userId] = { displayname: name, progress: 0, databaseUserId: databaseUserId};
  },
