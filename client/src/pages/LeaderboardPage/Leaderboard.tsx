...
export default function Leaderboard() {
	...
	const [sortedColumn, setSortedColumn] = useState(["max_wpm"]);
	...
	const sortByColumn = (column: TableKey) => {
		if (sortedColumn?.includes(column.toString())) {
			...
			const sortData = user_stats
				...
				.sort((a, b) => (a[column] > b[column] ? -1 : 1));
			setSortedData(sortData);
			setSortedColumn([column.toString()]);
			setBestFirst(true);
		}
	...
	const arrow = (sortBy: TableKey) => {
		return (
			<svg
				...
				${sortedColumn?.includes(sortBy) ? "rotate-180" : "rotate-0"} `}
	...
	return (
		<div className="h-full bg-white flex flex-col items-center justify-center gap-12">
			{loading ? (
				<div>Loading...</div>
			) : (
